...
<body>
<div id="app" class="index" :class="[theme, mobile]">
	<el-container>
		<!--左侧菜单-->
			...
			<el-scrollbar style="height:100%">
				<el-menu :default-active="asideMenuActive" class="el-menu-vertical-demo"
					...
					<div class="ms-admin-logo" :style="{display: 'flex','padding-left': (collapseMenu?'5px':'22px')}">
						<img :src="xh.base+'/statics/images/favicon.ico'"/>
		...
		<el-container class="ms-admin-container" v-cloak>
					...
					<el-submenu style="height: 100%;">
						...
						<div class="ms-admin-header-menu-all">
							<li class="ms-admin-header-menu-all-item" v-for="(item,index) of parentMenuList"
								...
								<i class="iconfont" :class="item.icon" style="padding-right: 4px"></i>
								<div style="width:80px">{{item.name}}</div>
								<div style="float: right;width: 18px;">
									<template>
										<i v-if="markList.find(function(x) {
                    return x.title == item.name
                })!=undefined" @click="cancelMarkMenu(item.name)"
										   class='el-icon-star-on'></i>
										<i v-else class='el-icon-star-off '
										   @click="markMenu(item.name,item.icon)"></i>
									</template>
								</div>
						...
						<el-submenu popper-class="ms-admin-header-menu-item">
							<template slot="title">
                                <span class="ms-admin-people-head"
                                      v-text="peopleInfo.nickName && peopleInfo.nickName.substr(peopleInfo.nickName.length - 2, peopleInfo.nickName.length)"></span>
								<span v-text='peopleInfo.username'></span>
							</template>
							<el-menu-item style="display: flex;align-items: center" @click.native='openModal'>
			...
			<el-main class="ms-admin-main">
				...
				<el-tabs class="ms-admin-tabs" v-model="currentTab" type="card" closable @tab-remove="closeTab"
				         @tab-click='tabClick'>
					<el-tab-pane v-for="(item, index) in editableTabs" :key="index" :label="item.name"
					             :name="item.name"
					             :data-modelId='item.modelId' :data-modelModelId='item.modelModelId'>
						<keep-alive>
							<iframe :src="item.url" :ref="item.name"></iframe>
						</keep-alive>
					</el-tab-pane>
				</el-tabs>
			</el-main>
...
<script>
	var indexVue = new Vue({
		...
		data: {
			...
			editableTabsValue: '',
			messageTypeList: [],
			editableTabs: [{"name": "工作台", "url": "main.html"}], //当前打开的tab页面
			...
			markList: [
				...
				{title: "资源管理", icon: "icon-xitongguanli", hide: true},
				{title: "教育管理", icon: "icon-neirongguanli", hide: true},
				{title: "招生管理", icon: "icon-huiyuanzhongxin", hide: true},
				{title: "客户管理", icon: "icon-zidingyiguanli", hide: true}
			],
			//登录用户信息
			peopleInfo: {
				username: '',
		...
		watch: {
			menuList: function (n, o) {
				...
				n && n.forEach(function (item, index) {
					item.modelModelId >= 1 ? that.subMenuList.push(item) : that.parentMenuList.push(item)
				});
			},
		...
		methods: {
			...
			list: function () {
				var that = this;
				xh.http.get(xh.base + "/sys/menu/nav").then(function (data) {
			...
			open: function (sub) {
				var that = this;
				var result = this.editableTabs.some(function (item, index) {
			...
			openMenu: function (menu, index) {
				...
				// this.getSubMenu(menu.modelId)[0] && this.$refs.menu.open(this.getSubMenu(menu.modelId)[0].name);
				var children = [];
			...
			managerGet: function () {
				var that = this;
				xh.http.get(xh.base + "/sys/user/info").then(function (data) {
					that.peopleInfo = data.user;
					resetPasswordVue.resetPasswordForm.username = data.user.username;
				}, function (err) {
					that.$message.error(err);
				})
			},
			exitSystem: function () {
				exitSystemVue.isShow = true;
			},
			//  打开修改密码，退出的模态框
			openModal: function () {
		...
		created: function () {
			this.collapseMenu = isMobile();
			this.getAuthorization();
			this.initMobileClass();
